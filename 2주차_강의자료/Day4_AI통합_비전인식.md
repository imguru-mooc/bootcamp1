...
> **방식** 따라하기(hands-on) · **분량** 실습 4-1 ~ 4-6
...
$ pip3 install mediapipe              # 손·얼굴·포즈 인식 (실습 4-6)
...
# 실습 4-6 · 다양한 AI 인터랙션 (제스처·표정·포즈)

**목표** 객체 검출 외에도 **사람의 동작 자체를 입력**으로 쓴다. MediaPipe로 손가락·표정·포즈를 인식해 하드웨어를 제어한다. 세 노드 모두 노트북에서 `/image_raw`를 구독하므로 `webcam_pub.py`만 떠 있으면 된다.

![다양한 AI 인터랙션 매핑](images/day4_ai_interaction.png)

> 💡 MediaPipe는 손(Hands)·얼굴(FaceMesh)·포즈(Pose)의 랜드마크(관절·특징점) 좌표를 실시간으로 줍니다. 그 좌표의 기하 관계만으로 제스처·표정을 판별하므로, 별도 학습 없이 가볍게 동작합니다.

## 4-6-1. 손 제스처 — 손가락 개수 → 7세그·모터

펴진 손가락 개수를 세어 7세그먼트에 숫자로 표시하고, 개수에 비례해 모터 속도를 정합니다(주먹=정지, 다섯=최대).

`~/ros2_labs/gesture_control.py` (노트북)

```python
#!/usr/bin/env python3
# 〔노트북〕 /image_raw → MediaPipe 손 인식 → 펴진 손가락 개수로 하드웨어 제어
import cv2
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image
...
class GestureControl(Node):
    def __init__(self):
        ...
        self.sub = self.create_subscription(Image, 'image_raw', self.on_image, 10)
        self.num_pub = self.create_publisher(Int32, 'number', 10)     # 7세그
        self.cmd_pub = self.create_publisher(Twist, 'cmd_vel', 10)    # 모터
        self.img_pub = self.create_publisher(Image, 'gesture/image', 10)
        self.get_logger().info('제스처 제어 시작 — /image_raw 구독')

    def count_fingers(self, lm, handedness):
        fingers = 0
        if handedness == 'Right':                       # 엄지: x 비교(손 방향별)
            fingers += 1 if lm[4].x < lm[3].x else 0
        else:
            fingers += 1 if lm[4].x > lm[3].x else 0
        for tip, pip in zip(TIPS[1:], PIPS[1:]):        # 나머지: 끝이 관절보다 위면 펴짐
            fingers += 1 if lm[tip].y < lm[pip].y else 0
        return fingers
    ...
    def on_image(self, msg):
        ...
        self.num_pub.publish(Int32(data=count))         # 손가락 개수 → 7세그
        cmd = Twist()
        cmd.linear.x = count / 5.0                       # 0=정지 ~ 5=최대 속도
        self.cmd_pub.publish(cmd)
...
```bash
# 〔노트북〕 (webcam_pub 실행 중)
$ python3 ~/ros2_labs/gesture_control.py
...
손가락을 0~5개 펴면 7세그에 숫자가 뜨고 모터 속도가 바뀝니다.

> 💡 MediaPipe는 셀카처럼 좌우가 뒤집혀 보일 수 있어, 엄지 판별의 좌/우가 반대로 나오면 `count_fingers`의 부등호를 뒤집으세요.

## 4-6-2. 얼굴 표정 — 미소·놀람 → RGB·부저·LCD

입 모양(가로 폭·세로 벌림)의 비율로 미소·놀람·무표정을 구분해 표시장치로 반응합니다.

`~/ros2_labs/expression_control.py` (노트북)

```python
#!/usr/bin/env python3
# 〔노트북〕 /image_raw → MediaPipe FaceMesh → 표정(미소/놀람/무표정)으로 하드웨어 제어
import cv2
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image
...
class ExpressionControl(Node):
    def __init__(self):
        super().__init__('expression_control')
        self.bridge = CvBridge()
        self.face = mp.solutions.face_mesh.FaceMesh(
            max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.6)
        self.sub = self.create_subscription(Image, 'image_raw', self.on_image, 10)
        self.status_pub = self.create_publisher(String, 'status', 10)    # RGB·부저
        self.lcd_pub = self.create_publisher(String, 'lcd_text', 10)     # LCD
        self.img_pub = self.create_publisher(Image, 'face/image', 10)
        self.get_logger().info('표정 제어 시작 — /image_raw 구독')
    ...
    def on_image(self, msg):
        ...
        if result.multi_face_landmarks:
            ...
            mouth_w = abs(lm[291].x - lm[61].x)      # 입 좌우 폭
            mouth_h = abs(lm[14].y - lm[13].y)       # 입 상하 벌림
            face_w = abs(lm[454].x - lm[234].x)      # 얼굴 폭(정규화 기준)
            ratio_w = mouth_w / face_w
            ratio_h = mouth_h / face_w
            if ratio_h > 0.18:
                expr = 'surprise'                    # 입 크게 벌림
            elif ratio_w > 0.42:
                expr = 'smile'                       # 입 옆으로 넓음
            else:
                expr = 'neutral'
        status = {'smile': 'ok', 'surprise': 'warn', 'neutral': 'off'}.get(expr, 'off')
        self.status_pub.publish(String(data=status))
        self.lcd_pub.publish(String(data=f'expr: {expr}'))
        cv2.putText(frame, expr, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
        self.img_pub.publish(self.bridge.cv2_to_imgmsg(frame, 'bgr8'))

def main(args=None):
    rclpy.init(args=args)
    node = ExpressionControl()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
...
```bash
# 〔노트북〕 (webcam_pub 실행 중)
$ python3 ~/ros2_labs/expression_control.py
# 〔Pi〕 표시장치 (Day 3)
$ python3 ~/ros2_labs/display_hub.py
$ python3 ~/ros2_labs/lcd_display.py
```

미소를 지으면 초록(`ok`), 입을 크게 벌리면 노랑·부저(`warn`), 무표정이면 꺼집니다. LCD에는 표정 이름이 표시됩니다.

> 💡 `ratio_h`·`ratio_w`의 임계값(0.18·0.42)은 사람·거리마다 다릅니다. `expr` 값을 로그로 보며 본인 얼굴 기준으로 보정하세요. 명명된 감정(행복/슬픔/분노 등)이 필요하면 `deepface`·`fer` 같은 감정 분류 라이브러리로 확장할 수 있습니다(도전 과제).

## 4-6-3. 전신 포즈 — 팔 동작 → 주행 명령

양팔의 들림 상태로 좌·우·정지·전진을 지시합니다(왼팔↑=좌회전, 오른팔↑=우회전, 양팔↑=정지, 내림=전진).

`~/ros2_labs/pose_drive.py` (노트북)

```python
#!/usr/bin/env python3
# 〔노트북〕 /image_raw → MediaPipe Pose → 팔 동작으로 주행 명령(/cmd_vel)
import cv2
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image
...
L_SH, R_SH, L_WR, R_WR = 11, 12, 15, 16   # 어깨/손목 랜드마크 인덱스

class PoseDrive(Node):
    def __init__(self):
        super().__init__('pose_drive')
        self.bridge = CvBridge()
        self.pose = mp.solutions.pose.Pose(min_detection_confidence=0.6)
        self.draw = mp.solutions.drawing_utils
        self.sub = self.create_subscription(Image, 'image_raw', self.on_image, 10)
        self.cmd_pub = self.create_publisher(Twist, 'cmd_vel', 10)
        self.gesture_pub = self.create_publisher(String, 'pose_gesture', 10)
        self.img_pub = self.create_publisher(Image, 'pose/image', 10)
        self.get_logger().info('포즈 주행 시작 — /image_raw 구독')
    ...
    def on_image(self, msg):
        ...
        if result.pose_landmarks:
            ...
            l_up = lm[L_WR].y < lm[L_SH].y       # 손목이 어깨보다 위면 팔 든 것
            r_up = lm[R_WR].y < lm[R_SH].y
            if l_up and r_up:
                gesture = 'stop'
            elif l_up:
                gesture = 'left';  cmd.angular.z = 0.8
            elif r_up:
                gesture = 'right'; cmd.angular.z = -0.8
            else:
                gesture = 'forward'; cmd.linear.x = 0.3
            self.draw.draw_landmarks(frame, result.pose_landmarks,
                                     mp.solutions.pose.POSE_CONNECTIONS)
        self.cmd_pub.publish(cmd)
        self.gesture_pub.publish(String(data=gesture))
        cv2.putText(frame, gesture, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 2)
        self.img_pub.publish(self.bridge.cv2_to_imgmsg(frame, 'bgr8'))
...
```bash
# 〔노트북〕 (webcam_pub 실행 중)
$ python3 ~/ros2_labs/pose_drive.py
$ ros2 run rqt_image_view rqt_image_view        # /pose/image
# 〔Pi〕 모터 (Day 3)
$ python3 ~/ros2_labs/dc_motor_node.py
```

> 💡 카메라는 좌우가 뒤집혀 보이므로(셀카 모드), 화면 기준 좌/우와 실제 좌/우가 반대일 수 있습니다. 의도와 반대로 돈다면 `angular.z` 부호를 바꾸거나 영상을 좌우 반전(`cv2.flip`)하세요. 카메라로 사람을 향하는 로봇이라 **장애물 가드(Day 3-4)** 와 함께 쓰면 안전합니다.

> ✅ **체크포인트 4-6**
> - [ ] 손가락 개수가 7세그에 표시되고 모터 속도에 반영
> - [ ] 미소·놀람·무표정에 따라 RGB·부저·LCD 반응
> - [ ] 팔 동작(좌/우/정지/전진)으로 모터 주행 명령 생성
...
5. 특정 손가락 제스처(예: 주먹=정지, 손바닥=전진, 검지=좌회전)를 정의해 `gesture_control`이 `/cmd_vel`로 주행을 지시하게 하라.

### 도전
6. YOLO 검출과 색상 추적을 런치 파일 하나로 묶고, 파라미터로 둘 중 하나를 선택하게 하라.
7. (4-5) Pi LiteRT FPS와 노트북 YOLO FPS를 같은 장면에서 측정해 표·그래프로 비교 보고서를 작성하라.
8. 표정 제어를 `deepface`/`fer` 등 감정 분류로 확장해, 명명된 감정(행복·놀람·화남)에 따라 다른 하드웨어 반응을 매핑하라.
...
| `mediapipe` 설치 실패 | 파이썬 버전 비호환 | Python 3.8~3.11 권장, `pip3 install mediapipe` 재시도 |
| 손가락/좌우가 반대 | 셀카 좌우 반전 | `cv2.flip(frame,1)` 또는 부등호·부호 반전 |
| 표정이 안 바뀜 | 임계값이 얼굴과 안 맞음 | `expr` 로그 보며 `ratio` 임계값 보정 |
...
# ── 4-6 다양한 AI 인터랙션 (노트북: 인식 / Pi: 하드웨어) ──
pip3 install mediapipe                            # 노트북
python3 ~/ros2_labs/gesture_control.py            # 손가락 → /number, /cmd_vel
python3 ~/ros2_labs/expression_control.py         # 표정 → /status, /lcd_text
python3 ~/ros2_labs/pose_drive.py                 # 팔 동작 → /cmd_vel
# Pi: seg_display / dc_motor_node / display_hub / lcd_display (Day 3)
```
